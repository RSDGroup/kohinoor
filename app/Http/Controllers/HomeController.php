...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $backgroundPatterns = [
            ['rect' => '#f8d7da', 'circle' => '#e66a77'],
            ['rect' => '#d4edda', 'circle' => '#28a745'],
            ['rect' => '#d1ecf1', 'circle' => '#17a2b8'],
            ['rect' => '#fff3cd', 'circle' => '#ffc107'],
            ['rect' => '#fce8e6', 'circle' => '#ea4335'],
            ['rect' => '#e8f0fe', 'circle' => '#4285f4'],
            ['rect' => '#f3e5f5', 'circle' => '#9c27b0'],
            ['rect' => '#f1f8e9', 'circle' => '#689f38'],
        ];

        $products = Item::where('is_approved',1)->latest()->take(8)->get();
        foreach ($products as $index => $product) {
            $patternIndex = $index % count($backgroundPatterns);
            $product->patternId = "pattern{$index}";
            $product->patternColors = $backgroundPatterns[$patternIndex];
        }
        if(isset($config) && $config){

            return view('home',compact('landing_data' ,'new_user' , 'products' , 'backgroundPatterns'));
